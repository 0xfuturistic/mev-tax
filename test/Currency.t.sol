...
contract CurrencyTest {
    function approve(Currency currency, address spender, uint256 amount) external {
        MockERC20(Currency.unwrap(currency)).approve(spender, amount);
    }
    ...
    function transferFrom(Currency currency, address from, address to, uint256 amount) external {
        currency.transferFrom(from, to, amount);
    }
...
contract TestCurrency is Test {
    ...
    function test_fuzz_transferFrom_native(uint256 amount) public {
        uint256 balanceBefore = otherAddress.balance;
        uint256 contractBalanceBefore = address(currencyTest).balance;

        if (amount <= contractBalanceBefore) {
            currencyTest.transferFrom(nativeCurrency, address(this), otherAddress, amount);
            assertEq(otherAddress.balance, balanceBefore + amount);
            assertEq(address(currencyTest).balance, contractBalanceBefore - amount);
        } else {
            vm.expectRevert(CurrencyLibrary.NativeTransferFailed.selector);
    ...
    function test_fuzz_transferFrom_token(uint256 amount) public {
        uint256 balanceBefore = currencyTest.balanceOf(erc20Currency, otherAddress);

        if (amount <= initialERC20Balance) {
            currencyTest.approve(erc20Currency, address(currencyTest), amount);
            currencyTest.transferFrom(erc20Currency, address(currencyTest), otherAddress, amount);
            assertEq(currencyTest.balanceOf(erc20Currency, otherAddress), balanceBefore + amount);
            assertEq(
                MockERC20(Currency.unwrap(erc20Currency)).balanceOf(address(currencyTest)), initialERC20Balance - amount
            );
